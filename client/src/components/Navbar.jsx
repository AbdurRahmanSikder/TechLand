...
const Navbar = () => {
    ...
    return (
        <nav className="flex items-center justify-between px-6 md:px-16 lg:px-24 xl:px-32 py-4 border-b border-gray-300 bg-primary relative transition-all">

            <div className="flex items-center justify-between gap-4">
                <NavLink to="/" onClick={() => setOpen(false)}>
                    <img className="h-9" src={assets.logo} alt="dummyLogoColored" />
                </NavLink>
                <div className="hidden lg:flex items-center text-sm gap-2 border rounded-sm  px-3 bg-white w-[100px] md:w-[200px] lg:w-[300px] xl:w-[400px]">
            ...
            <div className="hidden sm:flex items-center gap-8 text-white">
                ...
                {!user ? <button onClick={() => setShowUserLogin(true)} className="cursor-pointer px-8 py-2 bg-primary-dull hover:bg-gray-800 transition text-white rounded-full">
                    Login
            ...
            <div className='flex items-center gap-6 sm:hidden'>
                ...
                <button onClick={() => open ? setOpen(false) : setOpen(true)} aria-label="Menu" className="">
                    {/* Menu Icon SVG */}
                    <img src={assets.menu_icon} className='invert brightness-0' alt='menu' />
                </button>
            </div>
            {/* Mobile Menu */}
            {open &&
